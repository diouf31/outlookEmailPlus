...
def init_db(database_path: Optional[str] = None):
    ...
    try:
        ...
        # v25: 临时邮箱接入邮箱池 — temp_emails 新增池生命周期字段
        ...
        # 注意：上游 v2.8.0 将该迁移编号为 v24；本仓库 v24 已用于多用户，故顺延为 v25。
        cursor.execute("PRAGMA table_info(temp_emails)")
        temp_email_columns_v25 = [col[1] for col in cursor.fetchall()]
        for col_def in [
            ("pool_status", "TEXT DEFAULT NULL"),
            ("claimed_by", "TEXT DEFAULT NULL"),
            ("claimed_at", "TEXT DEFAULT NULL"),
            ...
            if col_def[0] not in temp_email_columns_v25:
                cursor.execute(f"ALTER TABLE temp_emails ADD COLUMN {col_def[0]} {col_def[1]}")
        cursor.execute("""
