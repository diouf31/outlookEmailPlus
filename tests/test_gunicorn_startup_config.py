...
class GunicornStartupConfigTests(unittest.TestCase):
    ...
    @unittest.skipIf(_IS_WINDOWS, "start-gunicorn.sh 仅在 Linux/Docker 环境执行")
    ...
    def test_start_script_rejects_zero_or_non_numeric_values(self):
        ...
        if _IS_WINDOWS:
            # Windows 无法直接执行 shebang shell 脚本；静态检查校验逻辑是否存在
            content = _read("scripts/start-gunicorn.sh")
            self.assertIn("require_positive_int GUNICORN_THREADS", content)
            self.assertIn("require_positive_int GUNICORN_WORKERS", content)
            self.assertIn("must be a positive integer", content)
            return
